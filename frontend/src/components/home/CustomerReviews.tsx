      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {reviews.slice(0, 3).map((review) => (
            <div
              key={review.id}
              className="bg-[var(--color-brand-soft)]/40 rounded-3xl p-6 border border-gray-100 flex flex-col justify-between hover:border-[var(--color-brand-bright)]/40 hover:shadow-lg transition-all"
            >
              <div>
                  ...
                  <div className="flex items-center gap-1">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        ...
                        className={`w-4 h-4 ${
                          i < review.rating
                            ? 'fill-[var(--color-brand-bright)] text-[var(--color-brand-bright)]'
