  ...
  const pillars = [
    {
      ...
      icon: <ShieldCheck className="w-6 h-6 text-[var(--color-brand-bright)]" />,
    },
    {
  ...
  return (
    <section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-[var(--color-brand-dark)] rounded-3xl sm:rounded-[2.5rem] p-8 sm:p-12 lg:p-16 text-white relative overflow-hidden shadow-2xl border border-[var(--color-brand-bright)]/20">
          {/* Subtle background glow */}
          <div className="absolute top-0 right-0 w-80 h-80 rounded-full bg-[var(--color-brand)]/15 blur-3xl pointer-events-none" />

          <div className="max-w-3xl mb-12">
            <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-white/10 text-[var(--color-brand-light)] text-xs font-bold mb-4 border border-[var(--color-brand-bright)]/30">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {pillars.map((pillar) => (
              ...
              >
                <div className="w-12 h-12 rounded-xl bg-white/10 flex items-center justify-center mb-4 border border-[var(--color-brand-bright)]/30">
                  {pillar.icon}
                </div>
                <h3 className="font-bold text-base text-white mb-2 font-['Outfit']">{pillar.title}</h3>
                <p className="text-xs text-[var(--color-brand-light)]/70 leading-relaxed">{pillar.desc}</p>
              </div>
            ))}
          </div>
        </div>
