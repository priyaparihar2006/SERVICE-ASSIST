import { Category, Service, Professional, Review, Coupon } from '../types';

export const CITIES = [
  { id: 'agra', name: 'Agra', state: 'Uttar Pradesh', active: true },
  { id: 'delhi', name: 'Delhi NCR', state: 'Delhi', active: true },
  { id: 'noida', name: 'Noida', state: 'Uttar Pradesh', active: true },
  { id: 'gurgaon', name: 'Gurgaon', state: 'Haryana', active: true },
  { id: 'lucknow', name: 'Lucknow', state: 'Uttar Pradesh', active: true },
  { id: 'jaipur', name: 'Jaipur', state: 'Rajasthan', active: true },
  { id: 'kanpur', name: 'Kanpur', state: 'Uttar Pradesh', active: true },
  { id: 'mathura', name: 'Mathura', state: 'Uttar Pradesh', active: true },
];

export const CITY_NAMES = CITIES.map((c) => c.name);

const RAW_CATEGORIES: Category[] = [
  {
    id: 'cat-laptop-computer',
    slug: 'laptop-and-computer',
    name: 'Laptop & Computer',
    icon: 'Laptop',
    description: 'Laptop repairs, software help, upgrades and home network setup',
    color: '#2563EB',
    bgPastel: '#DBEAFE',
    image: 'https://images.unsplash.com/photo-1588872657578-7efd1f1555ed?auto=format&fit=crop&w=800&q=80',
    servicesCount: 28,
  },
  {
    id: 'cat-electronics',
    slug: 'electronics',
    name: 'Electronics',
    icon: 'Tv',
    description: 'Mobile, TV, security camera and smart home assistance',
    color: '#7C3AED',
    bgPastel: '#EDE9FE',
    image: 'https://images.unsplash.com/photo-1550751827-4bd374c3f58b?auto=format&fit=crop&w=800&q=80',
    servicesCount: 21,
  },
  {
    id: 'cat-ac-appliances',
    slug: 'ac-and-appliances',
    name: 'AC & Appliances',
    icon: 'AirVent',
    description: 'Jet pump servicing, gas charging, refrigerator & washing machine repair',
    color: '#FF7A00',
    bgPastel: '#FFF1E5',
    badge: 'Popular',
    image: '/service-images/srv-ac-foamjet.png',
    servicesCount: 17,
  },
  {
    id: 'cat-cleaning',
    slug: 'cleaning-and-pest',
    name: 'Home Cleaning',
    icon: 'Sparkles',
    description: 'Deep bathroom, kitchen, sofa shampooing & hospital-grade termite control',
    color: '#FF7A00',
    bgPastel: '#FFF1E5',
    badge: 'Trending',
    image: 'https://images.unsplash.com/photo-1581578731548-c64695cc6952?auto=format&fit=crop&w=800&q=80',
    servicesCount: 18,
  },
  {
    id: 'cat-beauty',
    slug: 'beauty-and-wellness',
    name: 'Beauty & Salon',
    icon: 'Scissors',
    description: 'Salon at home for women & men, facials, waxing, pedicure & reflexology',
    color: '#EC4899',
    bgPastel: '#FCE7F3',
    badge: 'Best Rated',
    image: 'https://images.unsplash.com/photo-1560066984-138dadb4c035?auto=format&fit=crop&w=800&q=80',
    servicesCount: 34,
  },
  {
    id: 'cat-electrician',
    slug: 'electrician-and-repairs',
    name: 'Electrician',
    icon: 'Zap',
    description: 'Switchboard fixes, inverter setup, ceiling fan installation & wiring',
    color: '#F59E0B',
    bgPastel: '#FEF3C7',
    image: 'https://images.unsplash.com/photo-1621905252507-b35492cc74b4?auto=format&fit=crop&w=800&q=80',
    servicesCount: 16,
  },
  {
    id: 'cat-plumber',
    slug: 'plumbing-services',
    name: 'Plumbing',
    icon: 'Wrench',
    description: 'Leak detection, tap & mixer installation, drainage clearing & geysers',
    color: '#3B82F6',
    bgPastel: '#DBEAFE',
    image: 'https://images.unsplash.com/photo-1585704032915-c3400ca199e7?auto=format&fit=crop&w=800&q=80',
    servicesCount: 14,
  },
  {
    id: 'cat-carpenter',
    slug: 'carpentry-and-furniture',
    name: 'Carpentry',
    icon: 'Hammer',
    description: 'Modular furniture assembly, door locks, hinges & customized wood repairs',
    color: '#8B5CF6',
    bgPastel: '#EDE9FE',
    image: 'https://images.unsplash.com/photo-1504148455328-c376907d081c?auto=format&fit=crop&w=800&q=80',
    servicesCount: 5,
  },
  {
    id: 'cat-pest-control',
    slug: 'pest-control',
    name: 'Pest Control',
    icon: 'ShieldAlert',
    description: 'Herbal odorless gel for cockroaches, anti-termite shield & bed bug removal',
    color: '#10B8A8',
    bgPastel: '#CCFBF1',
    badge: 'Warranty',
    image: 'https://images.unsplash.com/photo-1584820927498-cfe5211fd8bf?auto=format&fit=crop&w=800&q=80',
    servicesCount: 5,
  },
  {
    id: 'cat-painting',
    slug: 'painting-and-waterproofing',
    name: 'Painting & Walls',
    icon: 'Paintbrush',
    description: 'Full home whitewash, waterproof wall coatings, texture accent walls',
    color: '#FF7A00',
    bgPastel: '#FFF1E5',
    image: 'https://images.unsplash.com/photo-1589939705384-5185137a7f0f?auto=format&fit=crop&w=800&q=80',
    servicesCount: 4,
  },
  {
    id: 'cat-moving',
    slug: 'packers-and-movers',
    name: 'Packers & Movers',
    icon: 'Truck',
    description: 'Local and inter-city hassle-free relocation with bubble wrap protection',
    color: '#F97316',
    bgPastel: '#FFEDD5',
    image: 'https://images.unsplash.com/photo-1600585154526-990dced4db0d?auto=format&fit=crop&w=800&q=80',
    servicesCount: 4,
  },
  {
    id: 'cat-appliance-repair',
    slug: 'appliance-repair',
    name: 'Appliance Repair',
    icon: 'Wrench',
    description: 'Washing machine, refrigerator, microwave & kitchen chimney repair',
    color: '#E85D04',
    bgPastel: '#FFF1E5',
    badge: 'Same Day',
    image: 'https://images.unsplash.com/photo-1581092160607-ee22621dd758?auto=format&fit=crop&w=800&q=80',
    servicesCount: 10,
  },
  {
    id: 'cat-bathroom-cleaning',
    slug: 'bathroom-cleaning',
    name: 'Bathroom Cleaning',
    icon: 'Sparkles',
    description: 'Intense hard water stain removal, descaling, tile scrubbing & sanitization',
    color: '#3B82F6',
    bgPastel: '#DBEAFE',
    badge: 'Popular',
    image: '/service-images/srv-bathroom-deep.png',
    servicesCount: 3,
  },
  {
    id: 'cat-sofa-cleaning',
    slug: 'sofa-cleaning',
    name: 'Sofa Cleaning',
    icon: 'Sofa',
    description: 'Deep foam shampooing and extraction for fabric & leather sofas',
    color: '#8B5CF6',
    bgPastel: '#EDE9FE',
    image: 'https://images.unsplash.com/photo-1527515637462-cff94eecc1ac?auto=format&fit=crop&w=800&q=80',
    servicesCount: 3,
  },
  {
    id: 'cat-water-purifier',
    slug: 'water-purifier',
    name: 'Water Purifier',
    icon: 'Droplet',
    description: 'RO service, membrane filter replacement & TDS water purity check',
    color: '#0EA5E9',
    bgPastel: '#E0F2FE',
    image: '/service-images/ro-purifier/servicing.png',
    servicesCount: 4,
  },
  {
    id: 'cat-home-improvement',
    slug: 'home-improvement',
    name: 'Home Improvement',
    icon: 'Home',
    description: 'Modular interior upgrades, false ceiling, lighting & aesthetic repairs',
    color: '#FF7A00',
    bgPastel: '#FFF1E5',
    badge: 'Premium',
    image: 'https://images.unsplash.com/photo-1513694203232-719a280e022f?auto=format&fit=crop&w=800&q=80',
    servicesCount: 4,
  },
];

const FEATURED_PRIMARY_SERVICES: Service[] = [
  {
    id: 'srv-ac-foamjet',
    slug: 'ac-jet-service',
    name: 'Foam Jet Power AC Servicing',
    categoryId: 'cat-ac-appliances',
    categoryName: 'AC & Appliances',
    categoryImage: '/service-images/srv-ac-foamjet.png',
    subcategory: 'AC Services',
    priceType: 'FIXED',
    serviceType: 'HOME_VISIT',
    rating: 4.88,
    reviewsCount: 14200,
    startingPrice: 599,
    originalPrice: 899,
    durationMin: 45,
    image: '/service-images/srv-ac-foamjet.png',
    locations: CITY_NAMES,
    shortDesc: 'Deep foam cleaning of indoor cooling coils + high pressure jet pump spray for 2x faster cooling.',
    description: 'Revive your split or window air conditioner with our proprietary dual-foam technology. Eliminates accumulated dust, allergens, bacteria, and improves cooling efficiency while reducing electricity consumption by up to 25%.',
    popular: true,
    trending: true,
    recommendedReason: 'Recommended for hot seasons to boost cooling efficiency and reduce electricity bills',
    variants: [
      {
        id: 'var-split-1',
        name: 'Split AC Power Jet Clean (1 Unit)',
        price: 599,
        originalPrice: 899,
        durationMin: 45,
        description: 'Complete indoor jet foam cleaning + outdoor unit pressure wash + anti-bacterial coating.',
        included: ['Indoor coil foam spray', 'Jet pump wash', 'Drain pipe clearing', 'Gas pressure check', 'Filter sanitize'],
      },
      {
        id: 'var-split-2',
        name: 'Split AC Dual Combo (2 Units)',
        price: 1099,
        originalPrice: 1798,
        durationMin: 80,
        description: 'Comprehensive jet service for 2 Split ACs. Best value for master bedroom and living room.',
        included: ['Both indoor units foam jet wash', 'Both outdoor compressors cleaned', 'Gas pressure report', '30-day service warranty'],
      },
      {
        id: 'var-window-1',
        name: 'Window AC Deep Descaling',
        price: 499,
        originalPrice: 749,
        durationMin: 40,
        description: 'Full unmounting, high-pressure coil wash, tray descaling and re-installation.',
        included: ['Machine extraction', 'Tray wash', 'Motor lubrication check', 'Temp check test'],
      },
    ],
    whatIncluded: [
      'Indoor cooling coil foam cleaning with non-corrosive chemical',
      'High-pressure water jet spray of blower fan & drain tray',
      'Outdoor condenser unit pressure wash',
      'Gas leak detection & operating pressure check',
      'Pre & post service temperature check',
      'Mess-free catch jacket to protect walls and floor',
    ],
    whatExcluded: [
      'Cost of spare parts or capacitor replacement if faulty',
      'Refrigerant / Gas refill (available as an add-on)',
      'Masonry or carpentry repairs',
    ],
    whyChoose: [
      'Increases cooling airflow by up to 40%',
      'Saves up to ₹1,200 on monthly power bills',
      '30-day no-questions-asked service guarantee',
      'Verified technicians with 5+ years experience',
    ],
    steps: [
      'Pre-service inspection and cooling test',
      'Waterproof jacket mounting',
      'High-pressure foam spray and jet rinse',
      'Outdoor condenser wash',
      'Post-service performance calibration',
    ],
    faqs: [
      {
        question: 'Will the water make a mess on my bedroom wall?',
        answer: 'Not at all! Our technician uses a specialized waterproof spill jacket with a drainage funnel pipe directly into a bucket.',
      },
      {
        question: 'How often should I get my AC jet serviced?',
        answer: 'We recommend servicing before the summer peak and once mid-season for optimal cooling and lower power bills.',
      },
    ],
  },
  {
    id: 'srv-bathroom-deep',
    slug: 'bathroom-deep-cleaning',
    name: 'Intense Bathroom Deep Cleansing',
    categoryId: 'cat-bathroom-cleaning',
    categoryName: 'Bathroom Cleaning',
    categoryImage: '/service-images/srv-bathroom-deep.png',
    subcategory: 'Bathroom Deep Cleaning',
    priceType: 'FIXED',
    serviceType: 'HOME_VISIT',
    rating: 4.84,
    reviewsCount: 18450,
    startingPrice: 449,
    originalPrice: 699,
    durationMin: 60,
    image: '/service-images/srv-bathroom-deep.png',
    locations: CITY_NAMES,
    shortDesc: 'Hard water stain removal from tiles & glass, descaling of taps, WC sanitization & grout scrubbing.',
    description: 'Transform stained tiles, cloudy glass partitions, and scaled taps into sparkling sanitary perfection. We utilize non-toxic acid-free chemicals that restore shine without eroding tile grout or chrome finishes.',
    popular: true,
    trending: false,
    recommendedReason: 'Top booked service in your city for hard water scale removal',
    variants: [
      {
        id: 'var-bath-1',
        name: 'Single Bathroom Complete Scrub',
        price: 449,
        originalPrice: 699,
        durationMin: 60,
        description: 'Ideal for 1 master or guest bathroom with severe hard-water stains.',
        included: ['Wall & floor tile rotary scrub', 'Glass partition descaling', 'WC & washbasin sanitization', 'Mirror & fitting polish'],
      },
      {
        id: 'var-bath-2',
        name: '2 Bathrooms Refresh Pack',
        price: 799,
        originalPrice: 1398,
        durationMin: 110,
        description: 'Deep hygiene scrub for two bathrooms at a bundled discount.',
        included: ['Full scrub for 2 bathrooms', 'Anti-fungal treatment', 'Exhaust & geyser outer wipe'],
      },
    ],
    whatIncluded: [
      'Chemical descaling of hard water deposits on wall & floor tiles',
      'Shower glass panel stain extraction',
      'Toilet bowl (WC), bidet, and washbasin sanitization with hospital-grade disinfectant',
      'Mirror cleaning & chrome tap buffing',
      'Exhaust fan outer blade degreasing',
    ],
    whatExcluded: [
      'Ceiling paint peel restoration',
      'Unclogging severe main line blockages',
    ],
    whyChoose: [
      'Acid-free formulas protect expensive Jaquar & Kohler fixtures',
      'Removes up to 99.9% mildew and bacteria',
      'Eco-friendly child & pet safe hygiene chemicals',
    ],
    steps: [
      'Dry dust and debris vacuuming',
      'Hard water chemical application',
      'Rotary tile and grout scrubbing',
      'Chrome fixture descaling and buffing',
      'Steam disinfection and drying',
    ],
    faqs: [
      {
        question: 'Do your chemicals damage chrome taps or marble tiles?',
        answer: 'No, we never use raw hydrochloric acid. We use mild alkaline and enzymatic cleaners specifically formulated for CP fittings and natural stone.',
      },
    ],
  },
  {
    id: 'srv-salon-women',
    slug: 'salon-at-home-women',
    name: 'Glow & Relax Salon Package for Women',
    categoryId: 'cat-beauty',
    categoryName: 'Beauty & Salon',
    categoryImage: 'https://images.unsplash.com/photo-1560066984-138dadb4c035?auto=format&fit=crop&w=800&q=80',
    subcategory: 'Facial & Skincare',
    priceType: 'FIXED',
    serviceType: 'HOME_VISIT',
    rating: 4.92,
    reviewsCount: 21300,
    startingPrice: 899,
    originalPrice: 1499,
    durationMin: 90,
    image: 'https://images.unsplash.com/photo-1560066984-138dadb4c035?auto=format&fit=crop&w=800&q=80',
    locations: CITY_NAMES,
    shortDesc: 'O3+ Facial, honey chocolate waxing, classic pedicure, and threading in the comfort of home.',
    description: 'Indulge in a premium parlor experience without stepping outside. Our certified beauticians arrive with sanitized disposable sheets, sealed branded product pouches, and soothing spa music.',
    popular: true,
    trending: true,
    recommendedReason: 'Top rated by women in your neighborhood for hygiene and organic treatments',
    variants: [
      {
        id: 'var-salon-glow',
        name: 'Radiance Glow Combo (Facial + Waxing)',
        price: 899,
        originalPrice: 1499,
        durationMin: 75,
        description: 'Diamond brightening facial + full arms & underarms honey waxing + eyebrow shaping.',
        included: ['Diamond Facial', 'Full arms honey wax', 'Underarms wax', 'Eyebrow & upper lip threading'],
      },
      {
        id: 'var-salon-headtoeto',
        name: 'Deluxe Head-to-Toe Pampering',
        price: 1599,
        originalPrice: 2499,
        durationMin: 120,
        description: 'O3+ Gold peel facial + full legs & arms waxing + rose pedicure & manicure.',
        included: ['O3+ Gold Facial', 'Full legs waxing', 'Full arms waxing', 'Spa Pedicure', 'Spa Manicure'],
      },
    ],
    whatIncluded: [
      'Sealed single-use product sachets (L’Oréal, O3+, Cheryl’s)',
      '100% disposable bedsheets, gloves, and wooden spatulas',
      'Post-service cleanup with zero mess left behind',
      'Complimentary consultation by licensed cosmetologists',
    ],
    whatExcluded: ['Chemical hair smoothening / rebonding'],
    whyChoose: [
      'Rigorous 10-step sterilization protocol',
      'No waiting in salon queues',
      'Experienced beauticians background verified by police',
    ],
    steps: [
      'Consultation & skin type assessment',
      'Sanitized workstation setup',
      'Gentle cleansing & exfoliation',
      'Therapeutic facial massage & mask',
      'Neat post-service cleanup',
    ],
    faqs: [
      {
        question: 'Are the products genuine and authentic?',
        answer: 'Yes! All cosmetics are delivered in manufacturer-sealed mono-dosages and opened right in front of you.',
      },
    ],
  },
  {
    id: 'srv-electrician-visit',
    slug: 'electrician-on-demand',
    name: 'Electrician On-Demand & Diagnostics',
    categoryId: 'cat-electrician',
    categoryName: 'Electrician',
    categoryImage: 'https://images.unsplash.com/photo-1621905252507-b35492cc74b4?auto=format&fit=crop&w=800&q=80',
    subcategory: 'Electrical Repairs',
    priceType: 'INSPECTION',
    serviceType: 'HOME_VISIT',
    rating: 4.85,
    reviewsCount: 9700,
    startingPrice: 199,
    originalPrice: 299,
    durationMin: 30,
    image: 'https://images.unsplash.com/photo-1621905252507-b35492cc74b4?auto=format&fit=crop&w=800&q=80',
    locations: CITY_NAMES,
    shortDesc: 'Fan installation, socket repair, short circuit diagnosis, switchboard replacements & MCB tripping fixes.',
    description: 'Got a sparked socket, tripping MCB, or need a chandelier mounted? Our licensed electricians arrive with multi-meters, drills, and standard copper wiring to fix all electrical faults safely.',
    popular: true,
    trending: false,
    recommendedReason: 'Quickest doorstep response time in under 45 minutes',
    variants: [
      {
        id: 'var-elec-diag',
        name: 'Inspection & Minor Fix (Up to 30 mins)',
        price: 199,
        originalPrice: 299,
        durationMin: 30,
        description: 'Diagnostic visit for wiring checks, single switch/socket replacement, or bulb holder repair.',
        included: ['Inspection fee included', 'Diagnostic testing', 'Minor connection fixes'],
      },
      {
        id: 'var-elec-fan',
        name: 'Ceiling Fan Installation / Uninstallation',
        price: 249,
        originalPrice: 399,
        durationMin: 35,
        description: 'Mounting of ceiling fan on hook, regulator wiring, and balancing test.',
        included: ['Bracket assembly', 'Wiring connection', 'Safety balance test'],
      },
    ],
    whatIncluded: [
      'Safety check with voltage tester and insulated tools',
      'Fixing of reported electrical issue',
      'Clean drill dust cleanup',
      '30-day warranty on workmanship',
    ],
    whatExcluded: ['Cost of replacement switches, wires, or MCBs (billed on actual MRP)'],
    whyChoose: [
      'Certified and licensed electricians only',
      'Zero surprise billing with transparent rate cards',
      'Complimentary earthing check for household appliances',
    ],
    steps: [
      'Circuit safety isolation',
      'Fault diagnosis with multimeter',
      'Quotation for required replacement parts',
      'Repair and secure terminal tightening',
      'Live load & earthing verification',
    ],
    faqs: [
      {
        question: 'Can the electrician purchase parts for me?',
        answer: 'Yes! The electrician can procure ISI-marked switches or wires from authorized neighborhood retailers and provide the purchase receipt.',
      },
    ],
  },
  {
    id: 'srv-sofa-cleaning',
    slug: 'sofa-carpet-shampooing',
    name: 'Sofa & Upholstery Deep Shampooing',
    categoryId: 'cat-sofa-cleaning',
    categoryName: 'Sofa Cleaning',
    categoryImage: 'https://images.unsplash.com/photo-1527515637462-cff94eecc1ac?auto=format&fit=crop&w=800&q=80',
    subcategory: 'Sofa Cleaning',
    priceType: 'FIXED',
    serviceType: 'HOME_VISIT',
    rating: 4.89,
    reviewsCount: 11200,
    startingPrice: 699,
    originalPrice: 999,
    durationMin: 70,
    image: 'https://images.unsplash.com/photo-1555041469-a586c61ea9bc?auto=format&fit=crop&w=800&q=80',
    locations: CITY_NAMES,
    shortDesc: '3-stage dry vacuum, injection foam shampoo, and moisture extraction to eliminate stains & mites.',
    description: 'Give your living room couch a fresh, hygienic makeover. Removes tea/coffee stains, dust mites, pet hair, and musty odors using German high-suction extractors.',
    popular: true,
    trending: true,
    recommendedReason: 'Frequently booked with Home Cleaning for weekend home refreshes',
    variants: [
      {
        id: 'var-sofa-3',
        name: '3-Seater Fabric Sofa Shampooing',
        price: 699,
        originalPrice: 999,
        durationMin: 60,
        description: 'Deep extraction shampooing for a 3-seater fabric sofa.',
        included: ['Dry vacuum', 'Stain spot treatment', 'Foam brush shampoo', 'Suction moisture extraction'],
      },
      {
        id: 'var-sofa-5',
        name: '5-Seater (3+1+1 or L-Shape) Sofa',
        price: 999,
        originalPrice: 1499,
        durationMin: 90,
        description: 'Complete cleansing for 5 seating seats including cushions.',
        included: ['All seats and backrests', '5 complimentary cushions', 'Deodorization treatment'],
      },
    ],
    whatIncluded: [
      'Industrial dry vacuuming to remove deep-seated particles',
      'Non-hazardous chemical shampoo spray',
      'Gentle mechanical agitation for stubborn stains',
      'High-power moisture extraction (dries within 3-4 hours)',
    ],
    whatExcluded: ['Faded fabric color restoration', 'Torn fabric patchwork'],
    whyChoose: [
      'Removes 98% of deep fabric allergens & pet odors',
      'Quick drying formulation allows couch usage in just a few hours',
    ],
    steps: [
      'Dry vacuuming for dust extraction',
      'Pre-spotting of stubborn stains',
      'Enzymatic foam injection & brush scrub',
      'High-suction water extraction',
      'Fabric freshener spray',
    ],
    faqs: [
      {
        question: 'How long will the sofa take to dry?',
        answer: 'Our extraction machine sucks out 90% of the moisture. In a room with ceiling fan running, the sofa will be 100% dry within 3 to 4 hours.',
      },
    ],
  },
  {
    id: 'srv-plumbing-tap',
    slug: 'plumbing-tap-leakage-fix',
    name: 'Tap, Mixer & Drain Pipe Repair',
    categoryId: 'cat-plumber',
    categoryName: 'Plumbing',
    categoryImage: 'https://images.unsplash.com/photo-1585704032915-c3400ca199e7?auto=format&fit=crop&w=800&q=80',
    subcategory: 'Plumbing Repairs',
    priceType: 'FIXED',
    serviceType: 'HOME_VISIT',
    rating: 4.81,
    reviewsCount: 8400,
    startingPrice: 149,
    originalPrice: 249,
    durationMin: 30,
    image: 'https://images.unsplash.com/photo-1585704032915-c3400ca199e7?auto=format&fit=crop&w=800&q=80',
    locations: CITY_NAMES,
    shortDesc: 'Dripping tap fixes, internal washer replacement, kitchen sink waste pipe unclogging & angle valves.',
    description: 'Stop annoying water drips and wasteful leaks. Our plumbers carry washers, Teflon seals, spindle cartridges, and pipe wrenches to resolve kitchen and bathroom plumbing issues rapidly.',
    popular: false,
    trending: false,
    variants: [
      {
        id: 'var-plumb-tap',
        name: 'Tap / Angle Valve Replacement or Repair',
        price: 149,
        originalPrice: 249,
        durationMin: 30,
        description: 'Fixing 1 leaking tap, changing cartridge spindle or fitting a new angle stop cock.',
        included: ['Dismantling fitting', 'Spindle / washer fix', 'Pressure leak test'],
      },
    ],
    whatIncluded: [
      'Thorough inspection of water pressure lines',
      'Disassembly, cleaning of aerators and replacement of worn washers',
      'Teflon thread sealing to eliminate pinhole leaks',
    ],
    whatExcluded: ['Excavation of underground piping'],
    whyChoose: [
      'Prevents water wastage and water seepage behind walls',
      'Experienced in premium Jaquar, Hindware, and Grohe fittings',
    ],
    steps: [
      'Main line water shutoff check',
      'Disassembly of valve & cartridge inspection',
      'Replacement of faulty gasket / spindle',
      'Water pressure test',
    ],
    faqs: [
      {
        question: 'What if the tap needs a new brass spindle?',
        answer: 'The plumber will show you the worn internal part and provide a genuine brass spindle replacement with clear pricing before fitting.',
      },
    ],
  },
  {
    id: 'srv-pest-control',
    slug: 'cockroach-pest-control',
    name: 'Herbal Cockroach & Pest Shield',
    categoryId: 'cat-pest-control',
    categoryName: 'Pest Control',
    categoryImage: 'https://images.unsplash.com/photo-1584820927498-cfe5211fd8bf?auto=format&fit=crop&w=800&q=80',
    subcategory: 'Home Pest Control',
    priceType: 'FIXED',
    serviceType: 'HOME_VISIT',
    rating: 4.87,
    reviewsCount: 7600,
    startingPrice: 799,
    originalPrice: 1199,
    durationMin: 45,
    image: 'https://images.unsplash.com/photo-1581578731548-c64695cc6952?auto=format&fit=crop&w=800&q=80',
    locations: CITY_NAMES,
    shortDesc: '100% odorless herbal gel paste applied in kitchen cabinets + drain spray to eradicate cockroaches.',
    description: 'No need to empty your kitchen cabinets or leave your house! Our CIB-approved odorless gel is applied in tiny target dots in cabinet hinges, drawers, and behind refrigerators, targeting the nesting colony.',
    popular: true,
    trending: false,
    recommendedReason: 'Essential quarterly pest shield for disease-free kitchens',
    variants: [
      {
        id: 'var-pest-1bhk',
        name: '1-2 BHK Herbal Gel Treatment',
        price: 799,
        originalPrice: 1199,
        durationMin: 45,
        description: 'Complete odorless gel application across kitchen, bathrooms, and living area.',
        included: ['Kitchen cabinet gel bait', 'Sink drain treatment', 'Behind fridge & microwave dots', '60-day warranty'],
      },
    ],
    whatIncluded: [
      'Bayer Maxforce / synthetic pyrethroid odorless dots',
      'No requirement to vacate home or empty kitchen utensils',
      'Safe for infants, elderly family members, and pets',
      'Complimentary retreat within 60 days if pests reappear',
    ],
    whatExcluded: ['Bed bug thermal treatment (available as specialized service)'],
    whyChoose: ['Zero pungent chemical smell', 'Targets cockroach colonies via domino effect', '60-day service guarantee'],
    steps: [
      'Inspection of kitchen cracks & warm crevices',
      'Precision application of odorless gel dots',
      'Drain barrier chemical spray',
      'Guidance on preventive maintenance',
    ],
    faqs: [
      {
        question: 'Do I need to wash all kitchen utensils after the treatment?',
        answer: 'No! Because we do not use aerosol pesticide sprays, your utensils and food grains remain completely untouched and safe.',
      },
    ],
  },
  {
    id: 'srv-water-purifier-ro',
    slug: 'ro-water-purifier-service',
    name: 'RO Water Purifier Service & Filter Replacement',
    categoryId: 'cat-water-purifier',
    categoryName: 'Water Purifier',
    categoryImage: '/service-images/ro-purifier/servicing.png',
    subcategory: 'Water Purifier',
    priceType: 'FIXED',
    serviceType: 'HOME_VISIT',
    rating: 4.91,
    reviewsCount: 8200,
    startingPrice: 399,
    originalPrice: 599,
    durationMin: 40,
    image: '/service-images/ro-purifier/servicing.png',
    locations: CITY_NAMES,
    shortDesc: 'Complete TDS water quality test, sediment/carbon filter flush & leakage seal.',
    description: 'Protect your family with certified drinking water servicing. We test inlet and outlet TDS and check UV/UF membrane integrity.',
    popular: true,
    trending: false,
    variants: [
      {
        id: 'var-ro-service',
        name: 'Standard RO Maintenance Check',
        price: 399,
        originalPrice: 599,
        durationMin: 40,
        description: 'TDS calibration, pipe descaling and booster pump pressure test.',
        included: ['TDS testing', 'Pre-filter clean', 'Booster pressure check'],
      },
    ],
    whatIncluded: ['TDS testing before and after service', 'Sanitization of storage tank'],
    whatExcluded: ['Cost of replacement RO membrane or sediment filters'],
    whyChoose: ['Certified water lab grade testing instruments', 'Compatible with Kent, Aquaguard, Pureit'],
    steps: [
      'Inlet water TDS & pressure test',
      'Pre-filter sediment candle flush',
      'Membrane flow & pump electrical diagnostics',
      'Tank disinfection',
      'Output mineral & TDS recheck',
    ],
    faqs: [
      {
        question: 'What TDS level is safe for drinking?',
        answer: 'WHO and BIS suggest TDS below 300 ppm with optimal essential minerals is ideal.',
      },
    ],
  },
  {
    id: 'srv-appliance-washing-machine',
    slug: 'washing-machine-refrigerator-repair',
    name: 'Washing Machine & Refrigerator Repair',
    categoryId: 'cat-appliance-repair',
    categoryName: 'Appliance Repair',
    categoryImage: 'https://images.unsplash.com/photo-1581092160607-ee22621dd758?auto=format&fit=crop&w=800&q=80',
    subcategory: 'Appliances',
    priceType: 'INSPECTION',
    serviceType: 'HOME_VISIT',
    rating: 4.88,
    reviewsCount: 6540,
    startingPrice: 299,
    originalPrice: 499,
    durationMin: 45,
    image: 'https://images.unsplash.com/photo-1581092160607-ee22621dd758?auto=format&fit=crop&w=800&q=80',
    locations: CITY_NAMES,
    shortDesc: 'Motor vibration, spin tub issue, cooling leakage & PCB repair with 30-day warranty.',
    description: 'Expert diagnostic testing for top-load, front-load washing machines, frost-free and single door refrigerators of all major brands.',
    popular: true,
    trending: true,
    variants: [
      {
        id: 'var-appliance-diag',
        name: 'Comprehensive Diagnostic Visit',
        price: 299,
        originalPrice: 499,
        durationMin: 45,
        description: 'Multi-meter circuit check, motor test and quote before repair.',
        included: ['Diagnostics visit', 'Defect identification', 'Standard quote for parts'],
      },
    ],
    whatIncluded: [
      'Complete mechanical and electrical inspection',
      'Original spare parts estimate on MRP',
      '30-day post-service warranty',
    ],
    whatExcluded: ['Cost of spare parts or replacement PCB'],
    whyChoose: ['Certified appliance engineers', 'Zero hidden fees'],
    steps: [
      'Fault symptom demonstration',
      'Motor, drum & electrical PCB diagnosis',
      'Transparent quote for required parts',
      'Repair & operational test',
    ],
    faqs: [
      {
        question: 'How quickly can the technician arrive?',
        answer: 'Technicians typically arrive within 45-60 minutes across all serviced cities.',
      },
    ],
  },
  {
    id: 'srv-laptop-screen',
    slug: 'laptop-screen-replacement',
    name: 'Laptop Screen & Hardware Replacement',
    categoryId: 'cat-laptop-computer',
    categoryName: 'Laptop & Computer',
    categoryImage: 'https://images.unsplash.com/photo-1588872657578-7efd1f1555ed?auto=format&fit=crop&w=800&q=80',
    subcategory: 'Laptop Repair',
    priceType: 'INSPECTION',
    serviceType: 'PICKUP_OR_HOME_VISIT',
    rating: 4.86,
    reviewsCount: 5200,
    startingPrice: 299,
    originalPrice: 499,
    durationMin: 60,
    image: 'https://images.unsplash.com/photo-1588872657578-7efd1f1555ed?auto=format&fit=crop&w=800&q=80',
    locations: CITY_NAMES,
    shortDesc: 'Flickering display, cracked screen replacement, battery & motherboard diagnostics for HP, Dell, Lenovo, Apple.',
    description: 'Expert doorstep laptop diagnostic and screen replacement service. Compatible with all major brands with 90-day hardware warranty.',
    popular: true,
    trending: true,
    variants: [
      {
        id: 'var-laptop-diag',
        name: 'Doorstep Diagnostic Visit',
        price: 299,
        originalPrice: 499,
        durationMin: 60,
        description: 'Hardware diagnosis and itemized estimate for parts.',
        included: ['Hardware test', 'Thermal check', 'Part quote'],
      },
    ],
    whatIncluded: ['Doorstep pickup/visit', 'Hardware diagnostic inspection', 'Anti-static handling'],
    whatExcluded: ['Cost of replacement display panel or battery'],
    whyChoose: ['Genuine OEM replacement parts', '90-day warranty on new screens'],
    steps: ['Symptom verification', 'Display & ribbon cable test', 'Quotation', 'Installation and color calibration'],
    faqs: [
      {
        question: 'Is screen replacement done at home?',
        answer: 'Most standard LED screens are replaced right at your doorstep in under 45 minutes.',
      },
    ],
  },
  {
    id: 'srv-smart-home-cctv',
    slug: 'cctv-smart-home-setup',
    name: 'CCTV Camera & Smart Home Installation',
    categoryId: 'cat-electronics',
    categoryName: 'Electronics',
    categoryImage: 'https://images.unsplash.com/photo-1550751827-4bd374c3f58b?auto=format&fit=crop&w=800&q=80',
    subcategory: 'Smart Home',
    priceType: 'FIXED',
    serviceType: 'HOME_VISIT',
    rating: 4.9,
    reviewsCount: 3890,
    startingPrice: 399,
    originalPrice: 699,
    durationMin: 60,
    image: 'https://images.unsplash.com/photo-1557597774-9d273605dfa9?auto=format&fit=crop&w=800&q=80',
    locations: CITY_NAMES,
    shortDesc: 'Wireless CCTV setup, smart doorbell mounting, Wi-Fi pairing & mobile app live view configuration.',
    description: 'Secure your home with professional smart security camera installations. Includes angle adjustment, cloud/SD card setup, and smartphone live feed configuration.',
    popular: true,
    trending: true,
    variants: [
      {
        id: 'var-cctv-1',
        name: 'Single Smart Camera Mounting & Config',
        price: 399,
        originalPrice: 699,
        durationMin: 60,
        description: 'Drilling, power adapter wiring, Wi-Fi pairing & mobile app setup.',
        included: ['Wall mounting', 'Wi-Fi pairing', 'Mobile app setup', 'Motion zone calibration'],
      },
    ],
    whatIncluded: ['Precision wall drilling and mounting', 'Mobile app live view configuration', 'Motion alert testing'],
    whatExcluded: ['Cost of cameras, SD cards, or power cables'],
    whyChoose: ['Optimal angle alignment without blind spots', 'Clean concealed cable management'],
    steps: ['Optimal camera angle selection', 'Mounting bracket fixing', 'Power and Wi-Fi sync', 'App demonstration'],
    faqs: [
      {
        question: 'Do you configure cameras for multiple family members phones?',
        answer: 'Yes! The technician will configure the live feed on all required smartphones in your household.',
      },
    ],
  },
];

const EXPANDED_SERVICE_GROUPS: [string, string, string, number, number, string, string][] = [
  ['cat-beauty', 'Beauty & Salon', 'Hair Services', 349, 60, 'hair', "Women's Haircut|Men's Haircut|Kids Haircut|Hair Styling|Hair Spa|Hair Wash & Blow Dry|Hair Coloring|Hair Smoothening|Hair Straightening|Beard Styling|Beard Trimming|Head Massage"],
  ['cat-beauty', 'Beauty & Salon', 'Manicure & Pedicure', 399, 60, 'nails', 'Basic Manicure|Premium Manicure|Gel Manicure|Nail Art|Basic Pedicure|Spa Pedicure|Gel Pedicure|Foot Spa|Cuticle Care'],
  ['cat-beauty', 'Beauty & Salon', 'Facial & Skincare', 499, 60, 'skincare', 'Basic Facial|Glow Facial|Deep Cleansing Facial|Cleanup|De-Tan Treatment|Skin Care Consultation'],
  ['cat-beauty', 'Beauty & Salon', 'Beauty Services', 299, 45, 'beauty', 'Waxing|Full Body Waxing|Eyebrow Threading|Upper Lip Threading|Bridal Makeup|Party Makeup|Saree Draping'],
  ['cat-laptop-computer', 'Laptop & Computer', 'Laptop Repair', 299, 60, 'laptop', 'Laptop Screen Replacement|Laptop Keyboard Replacement|Laptop Battery Replacement|Laptop Charging Port Repair|Laptop Overheating Fix|Laptop Fan Cleaning|Laptop Hinge Repair|Laptop Motherboard Diagnosis|Laptop Speaker Repair|Laptop Webcam Repair|Laptop Touchpad Repair'],
  ['cat-laptop-computer', 'Laptop & Computer', 'Software Services', 399, 60, 'software', 'Windows Installation|Windows Activation Assistance|Driver Installation|Software Installation|Laptop Formatting|Virus & Malware Removal|Laptop Performance Optimization|Data Backup Assistance|Operating System Troubleshooting'],
  ['cat-laptop-computer', 'Laptop & Computer', 'Hardware & Upgrades', 299, 60, 'laptop', 'RAM Upgrade|SSD Upgrade|HDD Replacement|Laptop Cleaning|Desktop Assembly|Computer Repair|Printer Setup|Wi-Fi & Network Troubleshooting'],
  ['cat-electronics', 'Electronics', 'Mobile & Tablet', 299, 60, 'mobile', 'Mobile Screen Repair|Mobile Battery Replacement|Charging Port Repair|Software Troubleshooting|Tablet Repair'],
  ['cat-electronics', 'Electronics', 'Home Electronics', 399, 75, 'electronics', 'TV Installation|TV Wall Mounting|TV Repair|Speaker Installation|Home Theatre Setup|CCTV Installation|CCTV Troubleshooting'],
  ['cat-electronics', 'Electronics', 'Smart Home', 399, 60, 'smart-home', 'Smart Doorbell Installation|Smart Lock Installation|Smart Device Setup|Wi-Fi Router Installation|Smart TV Setup'],
  ['cat-electronics', 'Electronics', 'Other Electronics', 299, 60, 'electronics', 'Printer Repair|Scanner Setup|Gaming Console Cleaning|Computer Peripheral Setup'],
  ['cat-electrician', 'Electrician', 'Electrical Repairs', 299, 60, 'electrical', 'Switch & Socket Repair|Ceiling Fan Repair|Inverter Repair|Wiring Repair|Short Circuit Inspection|Geyser Electrical Repair'],
  ['cat-electrician', 'Electrician', 'Electrical Installation', 349, 60, 'electrical', 'Fan Installation|Light Installation|LED Light Installation|Chandelier Installation|Inverter Installation|MCB Replacement|Doorbell Installation|Exhaust Fan Installation'],
  ['cat-plumber', 'Plumbing', 'Plumbing Repairs', 299, 60, 'plumbing', 'Tap Repair|Wash Basin Repair|Toilet Repair|Water Leakage Repair|Pipe Repair|Drain Cleaning|Sink Blockage Removal'],
  ['cat-plumber', 'Plumbing', 'Plumbing Installation', 399, 75, 'plumbing', 'Tap Installation|Basin Installation|Toilet Installation|Shower Installation|Bathroom Fitting Installation'],
  ['cat-cleaning', 'Home Cleaning', 'Home Cleaning', 1499, 180, 'cleaning', 'Full Home Cleaning|Kitchen Deep Cleaning|Bathroom Deep Cleaning|Sofa Cleaning|Carpet Cleaning|Mattress Cleaning|Floor Cleaning|Window Cleaning|Water Tank Cleaning|Move-in Cleaning|Move-out Cleaning|Balcony Cleaning'],
  ['cat-ac-appliances', 'AC & Appliances', 'AC Services', 499, 75, 'ac', 'AC General Servicing|AC Deep Cleaning|AC Gas Refill|AC Installation|AC Uninstallation|AC Repair|AC Cooling Issue|AC Water Leakage Repair'],
  ['cat-ac-appliances', 'AC & Appliances', 'Appliances', 399, 75, 'appliances', 'Washing Machine Repair|Refrigerator Repair|Microwave Repair|Dishwasher Repair|Geyser Repair|Chimney Cleaning|RO Water Purifier Service|RO Installation|RO Repair'],
];

const inspectionRegex = /repair|replacement|refill|issue|diagnosis|troubleshooting|installation|upgrade|assembly|formatting|data backup|water leakage|drain cleaning|blockage/i;
const slugify = (name: string) => name.toLowerCase().replace(/&/g, 'and').replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');

export const SERVICE_IMAGE_MAP: Record<string, string> = {
  // AC & Appliances
  'srv-catalog-ac-appliances-ac-general-servicing': 'https://images.unsplash.com/photo-1621905251189-08b45d6a269e?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-ac-appliances-ac-deep-cleaning': 'https://images.unsplash.com/photo-1581094288338-2314dddb7ece?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-ac-appliances-ac-gas-refill': 'https://images.unsplash.com/photo-1581092160607-ee22621dd758?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-ac-appliances-ac-installation': 'https://images.unsplash.com/photo-1590756254933-2873d72a83b6?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-ac-appliances-ac-uninstallation': 'https://images.unsplash.com/photo-1504328345606-18bbc8c9d7d1?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-ac-appliances-ac-repair': 'https://images.unsplash.com/photo-1621905252507-b35492cc74b4?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-ac-appliances-ac-cooling-issue': '/service-images/srv-catalog-ac-appliances-ac-cooling-issue.png',
  'srv-catalog-ac-appliances-ac-water-leakage-repair': 'https://images.unsplash.com/photo-1585704032915-c3400ca199e7?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-ac-appliances-washing-machine-repair': 'https://images.unsplash.com/photo-1626806787461-102c1bfaaea1?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-ac-appliances-refrigerator-repair': 'https://images.unsplash.com/photo-1571175443880-49e1d25b2bc5?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-ac-appliances-microwave-repair': 'https://images.unsplash.com/photo-1574269909862-7e1d70bb8078?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-ac-appliances-dishwasher-repair': 'https://images.unsplash.com/photo-1585837575652-267c041d77d4?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-ac-appliances-geyser-repair': 'https://images.unsplash.com/photo-1507652313519-d4e9174996dd?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-ac-appliances-chimney-cleaning': 'https://images.unsplash.com/photo-1556911220-e15b29be8c8f?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-ac-appliances-ro-water-purifier-service': '/service-images/ro-purifier/servicing.png',
  'srv-catalog-ac-appliances-ro-installation': '/service-images/ro-purifier/installation.png',
  'srv-catalog-ac-appliances-ro-repair': '/service-images/ro-purifier/filter-replacement.png',

  // Beauty & Salon
  'srv-catalog-beauty-women-s-haircut': 'https://images.unsplash.com/photo-1562322140-8baeececf3df?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-men-s-haircut': 'https://images.unsplash.com/photo-1599351431202-1e0f0137899a?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-kids-haircut': 'https://images.unsplash.com/photo-1595867818082-083862f3d630?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-hair-styling': 'https://images.unsplash.com/photo-1522337360788-8b13dee7a37e?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-hair-spa': 'https://images.unsplash.com/photo-1519699047748-de8e457a634e?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-hair-wash-and-blow-dry': 'https://images.unsplash.com/photo-1580618672591-eb180b1a973f?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-hair-coloring': 'https://images.unsplash.com/photo-1605497788044-5a32c7078486?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-hair-smoothening': 'https://images.unsplash.com/photo-1527799820374-dcf8d9d4a388?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-hair-straightening': 'https://images.unsplash.com/photo-1560066984-138dadb4c035?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-beard-styling': 'https://images.unsplash.com/photo-1621605815971-fbc98d665033?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-beard-trimming': 'https://images.unsplash.com/photo-1517832606299-7ae9b720a186?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-head-massage': 'https://images.unsplash.com/photo-1544161515-4ab6ce6db874?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-basic-manicure': 'https://images.unsplash.com/photo-1604654894610-df63bc536371?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-premium-manicure': 'https://images.unsplash.com/photo-1610992015732-2449b76344bc?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-gel-manicure': 'https://images.unsplash.com/photo-1632345031435-8727f6897d53?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-nail-art': 'https://images.unsplash.com/photo-1519014816548-bf5fe059798b?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-basic-pedicure': 'https://images.unsplash.com/photo-1519415510236-718bdfcd89c8?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-spa-pedicure': 'https://images.unsplash.com/photo-1540555700478-4be289fbecef?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-gel-pedicure': 'https://images.unsplash.com/photo-1599940824399-b87987ceb72a?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-foot-spa': 'https://images.unsplash.com/photo-1515377905703-c4788e51af15?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-cuticle-care': 'https://images.unsplash.com/photo-1522337660859-02fbefca4702?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-basic-facial': 'https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-glow-facial': 'https://images.unsplash.com/photo-1512290900672-1f0239a2c37d?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-deep-cleansing-facial': 'https://images.unsplash.com/photo-1508214751196-bcfd4ca60f91?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-cleanup': 'https://images.unsplash.com/photo-1516975080664-ed2fc6a32937?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-de-tan-treatment': 'https://images.unsplash.com/photo-1573461160327-b450ce3d8e7f?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-skin-care-consultation': 'https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-waxing': 'https://images.unsplash.com/photo-1560750588-73207b1ef5b8?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-full-body-waxing': 'https://images.unsplash.com/photo-1519823551278-64ac92734fb1?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-eyebrow-threading': 'https://images.unsplash.com/photo-1588516903720-8ceb67f9ef84?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-upper-lip-threading': 'https://images.unsplash.com/photo-1526045612212-70caf35c14df?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-bridal-makeup': 'https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-party-makeup': 'https://images.unsplash.com/photo-1512496015851-a90fb38ba796?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-beauty-saree-draping': 'https://images.unsplash.com/photo-1610030469983-98e550d6193c?auto=format&fit=crop&w=800&q=80',

  // Electrician
  'srv-catalog-electrician-switch-and-socket-repair': 'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electrician-ceiling-fan-repair': 'https://images.unsplash.com/photo-1581092160607-ee22621dd758?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electrician-inverter-repair': 'https://images.unsplash.com/photo-1517420704952-d9f39e95b43e?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electrician-wiring-repair': 'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electrician-short-circuit-inspection': 'https://images.unsplash.com/photo-1581092162384-8987c1d64718?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electrician-geyser-electrical-repair': 'https://images.unsplash.com/photo-1581092580497-e0d23cbdf1dc?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electrician-fan-installation': 'https://images.unsplash.com/photo-1581092162384-8987c1d64718?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electrician-light-installation': 'https://images.unsplash.com/photo-1513506003901-1e6a229e2d15?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electrician-led-light-installation': 'https://images.unsplash.com/photo-1565814329452-e1efa11c5b89?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electrician-chandelier-installation': 'https://images.unsplash.com/photo-1540932239986-30128078f3c5?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electrician-inverter-installation': 'https://images.unsplash.com/photo-1517420704952-d9f39e95b43e?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electrician-mcb-replacement': 'https://images.unsplash.com/photo-1555963966-b7ae5404b6ed?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electrician-doorbell-installation': 'https://images.unsplash.com/photo-1558002038-1055907df827?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electrician-exhaust-fan-installation': 'https://images.unsplash.com/photo-1581094794329-c8112a89af12?auto=format&fit=crop&w=800&q=80',

  // Electronics
  'srv-catalog-electronics-mobile-screen-repair': 'https://images.unsplash.com/photo-1580910051074-3eb694886505?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-mobile-battery-replacement': 'https://images.unsplash.com/photo-1563770660941-20978e870e26?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-charging-port-repair': 'https://images.unsplash.com/photo-1597872200969-2b65d56bd16b?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-software-troubleshooting': 'https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-tablet-repair': 'https://images.unsplash.com/photo-1544244015-0df4b3ffc6b0?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-tv-installation': 'https://images.unsplash.com/photo-1593359677879-a4bb92f829d1?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-tv-wall-mounting': 'https://images.unsplash.com/photo-1522869635100-9f4c5e86aa37?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-tv-repair': 'https://images.unsplash.com/photo-1461151304267-38535e780c79?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-speaker-installation': 'https://images.unsplash.com/photo-1545454675-3531b543be5d?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-home-theatre-setup': 'https://images.unsplash.com/photo-1517604931442-7e0c8ed2963c?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-cctv-installation': 'https://images.unsplash.com/photo-1557597774-9d273605dfa9?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-cctv-troubleshooting': 'https://images.unsplash.com/photo-1541872703-74c5e44368f9?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-smart-doorbell-installation': 'https://images.unsplash.com/photo-1558002038-1055907df827?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-smart-lock-installation': 'https://images.unsplash.com/photo-1583847268964-b28dc8f51f92?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-smart-device-setup': 'https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-wi-fi-router-installation': 'https://images.unsplash.com/photo-1544197150-b99a580bb7a8?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-smart-tv-setup': 'https://images.unsplash.com/photo-1593784991095-a205069470b6?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-printer-repair': 'https://images.unsplash.com/photo-1612815154858-60aa4c59eaa6?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-scanner-setup': 'https://images.unsplash.com/photo-1588702547923-7093a6c3ba33?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-gaming-console-cleaning': 'https://images.unsplash.com/photo-1606813907291-d86efa9b94db?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-electronics-computer-peripheral-setup': 'https://images.unsplash.com/photo-1527443224154-c4a3942d3acf?auto=format&fit=crop&w=800&q=80',

  // Home Cleaning
  'srv-catalog-cleaning-full-home-cleaning': 'https://images.unsplash.com/photo-1581578731548-c64695cc6952?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-cleaning-kitchen-deep-cleaning': 'https://images.unsplash.com/photo-1556911220-e15b29be8c8f?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-cleaning-bathroom-deep-cleaning': 'https://images.unsplash.com/photo-1584622650111-993a426fbf0a?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-cleaning-sofa-cleaning': 'https://images.unsplash.com/photo-1555041469-a586c61ea9bc?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-cleaning-carpet-cleaning': 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-cleaning-mattress-cleaning': 'https://images.unsplash.com/photo-1631049307264-da0ec9d70304?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-cleaning-floor-cleaning': 'https://images.unsplash.com/photo-1628177142898-93e36e4e3a50?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-cleaning-window-cleaning': 'https://images.unsplash.com/photo-1513694203232-719a280e022f?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-cleaning-water-tank-cleaning': 'https://images.unsplash.com/photo-1504328345606-18bbc8c9d7d1?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-cleaning-move-in-cleaning': 'https://images.unsplash.com/photo-160058515526-990dced4db0d?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-cleaning-move-out-cleaning': 'https://images.unsplash.com/photo-1527515637462-cff94eecc1ac?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-cleaning-balcony-cleaning': 'https://images.unsplash.com/photo-1513694203232-719a280e022f?auto=format&fit=crop&w=800&q=80',

  // Laptop & Computer
  'srv-catalog-laptop-computer-laptop-screen-replacement': 'https://images.unsplash.com/photo-1588872657578-7efd1f1555ed?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-laptop-keyboard-replacement': 'https://images.unsplash.com/photo-1587829741301-dc798b83add3?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-laptop-battery-replacement': 'https://images.unsplash.com/photo-1593642632823-8f785ba67e45?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-laptop-charging-port-repair': 'https://images.unsplash.com/photo-1597872200969-2b65d56bd16b?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-laptop-overheating-fix': 'https://images.unsplash.com/photo-1563770660941-20978e870e26?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-laptop-fan-cleaning': 'https://images.unsplash.com/photo-1591799264318-7e6ef8ddb7ea?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-laptop-hinge-repair': 'https://images.unsplash.com/photo-1517336714731-489689fd1ca8?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-laptop-motherboard-diagnosis': 'https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-laptop-speaker-repair': 'https://images.unsplash.com/photo-1545454675-3531b543be5d?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-laptop-webcam-repair': 'https://images.unsplash.com/photo-1587829741301-dc798b83add3?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-laptop-touchpad-repair': 'https://images.unsplash.com/photo-1525547719571-a2d4ac8945e2?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-windows-installation': 'https://images.unsplash.com/photo-1517694712202-14dd9538aa97?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-windows-activation-assistance': 'https://images.unsplash.com/photo-1550751827-4bd374c3f58b?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-driver-installation': 'https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-software-installation': 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-laptop-formatting': 'https://images.unsplash.com/photo-1531403009284-440f080d1e12?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-virus-and-malware-removal': 'https://images.unsplash.com/photo-1563986768609-322da13575f3?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-laptop-performance-optimization': 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-data-backup-assistance': 'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-operating-system-troubleshooting': 'https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-ram-upgrade': 'https://images.unsplash.com/photo-1541029071515-84cc54f84dc5?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-ssd-upgrade': 'https://images.unsplash.com/photo-1597872200969-2b65d56bd16b?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-hdd-replacement': 'https://images.unsplash.com/photo-1531492746076-161ca9bcad58?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-laptop-cleaning': 'https://images.unsplash.com/photo-1525547719571-a2d4ac8945e2?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-desktop-assembly': 'https://images.unsplash.com/photo-1587202372775-e229f172b9d7?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-computer-repair': 'https://images.unsplash.com/photo-1588702547923-7093a6c3ba33?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-printer-setup': 'https://images.unsplash.com/photo-1612815154858-60aa4c59eaa6?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-laptop-computer-wi-fi-and-network-troubleshooting': 'https://images.unsplash.com/photo-1544197150-b99a580bb7a8?auto=format&fit=crop&w=800&q=80',

  // Plumbing
  'srv-catalog-plumber-tap-repair': 'https://images.unsplash.com/photo-1585704032915-c3400ca199e7?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-plumber-wash-basin-repair': 'https://images.unsplash.com/photo-1584622650111-993a426fbf0a?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-plumber-toilet-repair': 'https://images.unsplash.com/photo-1507652313519-d4e9174996dd?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-plumber-water-leakage-repair': 'https://images.unsplash.com/photo-1607472586893-edb57bdc0e39?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-plumber-pipe-repair': 'https://images.unsplash.com/photo-1542013936693-884638332954?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-plumber-drain-cleaning': 'https://images.unsplash.com/photo-1563453392212-326f5e854473?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-plumber-sink-blockage-removal': 'https://images.unsplash.com/photo-1521207418485-99c705420785?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-plumber-tap-installation': 'https://images.unsplash.com/photo-1585704032915-c3400ca199e7?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-plumber-basin-installation': 'https://images.unsplash.com/photo-1584622650111-993a426fbf0a?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-plumber-toilet-installation': 'https://images.unsplash.com/photo-1507652313519-d4e9174996dd?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-plumber-shower-installation': 'https://images.unsplash.com/photo-1552321554-5fefe8c9ef14?auto=format&fit=crop&w=800&q=80',
  'srv-catalog-plumber-bathroom-fitting-installation': 'https://images.unsplash.com/photo-1584622650111-993a426fbf0a?auto=format&fit=crop&w=800&q=80',
};

const EXPANDED_CATALOG_SERVICES: Service[] = EXPANDED_SERVICE_GROUPS.flatMap(([categoryId, categoryName, subcategory, basePrice, durationMin, , names]) =>
  names.split('|').map((name) => {
    const slug = slugify(name);
    const isInspection = inspectionRegex.test(name);
    const priceType = isInspection ? ('INSPECTION' as const) : ('FIXED' as const);
    const feeNote = isInspection ? 'The listed price covers an inspection visit. Parts and additional work are quoted after diagnosis and require approval.' : 'The listed price covers the standard package.';
    const shortDesc = `${name} at your doorstep with clear scope and upfront transparent visit pricing.`;
    const id = `srv-catalog-${categoryId.slice(4)}-${slug}`;
    const image = SERVICE_IMAGE_MAP[id] || (name === 'AC Cooling Issue' ? '/service-images/srv-catalog-ac-appliances-ac-cooling-issue.png' : `https://images.unsplash.com/photo-1581092160607-ee22621dd758?auto=format&fit=crop&w=800&q=80`);

    return {
      id,
      slug: `${slug}-${categoryId.slice(4)}`,
      name,
      categoryId,
      categoryName,
      subcategory,
      priceType,
      serviceType: (categoryId === 'cat-laptop-computer' || categoryId === 'cat-electronics' ? 'PICKUP_OR_HOME_VISIT' : 'HOME_VISIT') as const,
      rating: 4.85,
      reviewsCount: 320,
      startingPrice: basePrice,
      originalPrice: Math.round(basePrice * 1.4),
      durationMin,
      image,
      locations: CITY_NAMES,
      shortDesc,
      description: `${shortDesc} ${feeNote} Verified professionals assignation with satisfaction warranty.`,
      whatIncluded: isInspection ? ['On-site diagnosis by verified specialist', 'Itemized transparent quote before additional work'] : ['Standard doorstep service execution', 'Scope confirmation & post-work verification'],
      whatExcluded: isInspection ? ['Cost of replacement spare parts or extra hardware'] : ['Parts and optional add-ons'],
      whyChoose: ['Verified professional assignment', 'Upfront pricing with 30-day service warranty', 'Timely doorstep arrival'],
      faqs: [{ question: 'Are replacement parts included?', answer: isInspection ? 'No, spare parts are provided at genuine MRP with itemized approval before installation.' : 'Items explicitly listed in the package are included.' }],
      variants: [{
        id: `var-catalog-${categoryId.slice(4)}-${slug}`,
        name: isInspection ? 'Inspection & Diagnostic Visit' : 'Standard Service Package',
        price: basePrice,
        originalPrice: Math.round(basePrice * 1.4),
        durationMin,
        description: feeNote,
        included: isInspection ? ['Diagnosis and itemized quote'] : ['Standard doorstep service visit'],
      }],
      popular: false,
      trending: false,
    };
  })
);

// Merge featured primary services with full expanded catalog without duplicates
const existingSlugs = new Set(FEATURED_PRIMARY_SERVICES.map(s => s.slug));
const combinedServices: Service[] = [
  ...FEATURED_PRIMARY_SERVICES,
  ...EXPANDED_CATALOG_SERVICES.filter(s => !existingSlugs.has(s.slug) && !FEATURED_PRIMARY_SERVICES.some(f => f.name.toLowerCase() === s.name.toLowerCase()))
];

export const DEFAULT_SERVICES: Service[] = combinedServices;

// Dynamically compute exact servicesCount for every category from the exact same source of truth
export const DEFAULT_CATEGORIES: Category[] = RAW_CATEGORIES.map((cat) => {
  const matching = DEFAULT_SERVICES.filter((s) => {
    if (s.categoryId === cat.id) return true;
    if (cat.id === 'cat-cleaning' && ['cat-cleaning', 'cat-bathroom-cleaning', 'cat-sofa-cleaning', 'cat-pest-control'].includes(s.categoryId)) return true;
    if (cat.id === 'cat-ac-appliances' && ['cat-ac-appliances', 'cat-water-purifier', 'cat-appliance-repair'].includes(s.categoryId)) return true;
    return false;
  });

  return {
    ...cat,
    servicesCount: matching.length > 0 ? matching.length : 1,
  };
});

export const DEFAULT_PROFESSIONALS: Professional[] = [
  {
    id: 'pro-rahul',
    name: 'Rahul Sharma',
    phone: '+91 98765 43210',
    email: 'rahul.technician@serviceassist.in',
    profession: 'Senior HVAC & AC Specialist',
    categoryId: 'cat-ac-appliances',
    rating: 4.9,
    reviewsCount: 1240,
    completedJobs: 1380,
    experienceYears: 7,
    verified: true,
    avatar: '/images/professionals/ac-technician-1.jpg',
    bio: 'Certified Daikin & Voltas refrigeration engineer with over 7 years in split and window AC deep jet cleaning, inverter diagnosis, and zero-mess service.',
    skills: ['Jet Foam Cleansing', 'Gas Pressure Check', 'PCB Diagnostic', 'Inverter AC Troubleshooting'],
    certifications: ['National Skill Development Certificate - HVAC Grade A', 'Service Assist Verified Master Partner'],
    languages: ['Hindi', 'English'],
    serviceAreas: CITY_NAMES,
    availableDays: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'],
    workingHours: '08:00 AM - 08:00 PM',
    isAvailableToday: true,
  },
  {
    id: 'pro-priya',
    name: 'Priya Verma',
    phone: '+91 98111 22334',
    email: 'priya.salon@serviceassist.in',
    profession: 'Lead Aesthetician & Hair Stylist',
    categoryId: 'cat-beauty',
    rating: 4.95,
    reviewsCount: 890,
    completedJobs: 980,
    experienceYears: 6,
    verified: true,
    avatar: '/images/professionals/beauty-1.jpg',
    bio: 'Former Lakmé senior trainer specializing in organic facials, painless roll-on waxing, bridal styling, and relaxing head acupressure.',
    skills: ['O3+ Hydra Facial', 'Rica Brazilian Waxing', 'L’Oréal Spa', 'Bridal Makeover'],
    certifications: ['VLCC Advanced Cosmetology', 'CIDESCO Certified Aesthetician'],
    languages: ['Hindi', 'English'],
    serviceAreas: CITY_NAMES,
    availableDays: ['Mon', 'Tue', 'Wed', 'Fri', 'Sat', 'Sun'],
    workingHours: '09:00 AM - 07:30 PM',
    isAvailableToday: true,
  },
  {
    id: 'pro-amit',
    name: 'Amit Rawat',
    phone: '+91 97123 99887',
    email: 'amit.cleaning@serviceassist.in',
    profession: 'Deep Cleaning Master Crew Lead',
    categoryId: 'cat-cleaning',
    rating: 4.88,
    reviewsCount: 1650,
    completedJobs: 1820,
    experienceYears: 8,
    verified: true,
    avatar: '/images/professionals/cleaner-1.jpg',
    bio: 'Equipped with industrial Taski equipment, eco-certified chemicals, and German steam vacs for spotless bathroom descaling and sofa restoration.',
    skills: ['Taski Single Disc Scrubbing', 'Karcher Steam Sanitization', 'Fabric Extraction', 'Tile Grout Cleansing'],
    certifications: ['ISO 9001 Hygiene & Sanitation Specialist'],
    languages: ['Hindi', 'Punjabi'],
    serviceAreas: CITY_NAMES,
    availableDays: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
    workingHours: '07:30 AM - 07:00 PM',
    isAvailableToday: true,
  },
  {
    id: 'pro-vikram',
    name: 'Vikram Singh',
    phone: '+91 99234 55667',
    email: 'vikram.electrician@serviceassist.in',
    profession: 'Licensed Master Electrician',
    categoryId: 'cat-electrician',
    rating: 4.86,
    reviewsCount: 1120,
    completedJobs: 1400,
    experienceYears: 9,
    verified: true,
    avatar: '/images/professionals/electrician-1.jpg',
    bio: 'Government certified Grade 1 electrician with experience in high voltage domestic circuit breakers, smart home switches, and shock-prevention testing.',
    skills: ['MCB & Distribution Boards', 'Inverter Wiring', 'Smart Switch Integration', 'Concealed Conduit Repairs'],
    certifications: ['State Electrical Supervisory License', 'Bureau of Energy Efficiency Auditor'],
    languages: ['Hindi', 'English'],
    serviceAreas: CITY_NAMES,
    availableDays: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'],
    workingHours: '08:00 AM - 09:00 PM',
    isAvailableToday: true,
  },
  {
    id: 'pro-manoj',
    name: 'Manoj Kumar',
    phone: '+91 98450 12345',
    email: 'manoj.plumbing@serviceassist.in',
    profession: 'Sanitary & Pressure Line Plumber',
    categoryId: 'cat-plumber',
    rating: 4.82,
    reviewsCount: 940,
    completedJobs: 1100,
    experienceYears: 6,
    verified: true,
    avatar: '/images/professionals/plumber-1.jpg',
    bio: 'Expert in modern CP fittings (Jaquar, Kohler, Grohe), pressurized lines, concealed pipe leaks, and instant water heater servicing.',
    skills: ['CP Fittings Replacement', 'Pressure Motor Setup', 'Drain Spiral Augering', 'Geyser Thermal Cutout'],
    certifications: ['Indian Plumbing Association Certified'],
    languages: ['Hindi'],
    serviceAreas: CITY_NAMES,
    availableDays: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
    workingHours: '08:00 AM - 08:30 PM',
    isAvailableToday: true,
  },
];

export const DEFAULT_REVIEWS: Review[] = [
  {
    id: 'rev-1',
    bookingId: 'bk-demo-1',
    serviceId: 'srv-ac-foamjet',
    serviceName: 'Foam Jet Power AC Servicing',
    customerName: 'Ananya Sharma',
    customerCity: 'Delhi NCR',
    rating: 5,
    comment: 'Rahul arrived exactly on time with a full professional jet pump kit. The foam cleaning made my 4-year-old split AC run like brand new and cooling is noticeably faster now!',
    tags: ['On-time', 'Professional Kit', 'Great Cooling'],
    date: '2026-09-18T10:30:00Z',
    verifiedBooking: true,
  },
  {
    id: 'rev-2',
    bookingId: 'bk-demo-2',
    serviceId: 'srv-bathroom-deep',
    serviceName: 'Intense Bathroom Deep Cleansing',
    customerName: 'Rohit Kulkarni',
    customerCity: 'Noida',
    rating: 5,
    comment: 'The hard water stains on my glass shower partition were completely gone after the scrub. Worth every rupee for the spotless finish.',
    tags: ['Spotless Clean', 'Hard Water Stain Removal'],
    date: '2026-09-15T14:15:00Z',
    verifiedBooking: true,
  },
  {
    id: 'rev-3',
    bookingId: 'bk-demo-3',
    serviceId: 'srv-salon-women',
    serviceName: 'Glow & Relax Salon Package for Women',
    customerName: 'Meera Rajput',
    customerCity: 'Agra',
    rating: 5,
    comment: 'Priya was extremely gentle, hygienic and used sealed mono-dosages. The facial and pedicure experience at home was better than luxury parlors.',
    tags: ['Hygienic', 'Organic Products', 'Super Relaxing'],
    date: '2026-09-12T16:45:00Z',
    verifiedBooking: true,
  },
];

export const DEFAULT_COUPONS: Coupon[] = [
  {
    code: 'WELCOME150',
    discountType: 'FLAT',
    value: 150,
    minBookingAmount: 399,
    description: 'Flat ₹150 OFF on your first Service Assist service booking',
    expiry: '2026-12-31',
  },
  {
    code: 'CLEAN10',
    discountType: 'PERCENTAGE',
    value: 10,
    minBookingAmount: 500,
    maxDiscount: 200,
    description: '10% OFF on all Cleaning & Pest Control services',
    expiry: '2026-10-30',
    categoryIds: ['cat-cleaning', 'cat-bathroom-cleaning', 'cat-sofa-cleaning', 'cat-pest-control'],
  },
  {
    code: 'SALON200',
    discountType: 'FLAT',
    value: 200,
    minBookingAmount: 799,
    description: 'Flat ₹200 OFF on premium salon at home packages',
    expiry: '2026-11-15',
    categoryIds: ['cat-beauty'],
  },
  {
    code: 'WEEKEND50',
    discountType: 'FLAT',
    value: 50,
    minBookingAmount: 299,
    description: '₹50 instant cash discount for weekend slots',
    expiry: '2026-12-31',
  },
  {
    code: 'FESTIVE300',
    discountType: 'FLAT',
    value: 300,
    minBookingAmount: 1299,
    description: 'Flat ₹300 OFF on full home service combos over ₹1299',
    expiry: '2026-12-31',
  },
];

